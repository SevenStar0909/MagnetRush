...
/// <summary>
/// Player のHPが指定値以下になったとき、部位ごとの小さな継続VFXを再生する。
/// </summary>
...
public class PlayerLowHealthEffect : MonoBehaviour
{
    ...
    private sealed class BodyPartEffect
    {
        ...
        public Vector3 LocalEulerOffset => m_localEulerOffset;
    ...
    [Label("エフェクトPrefab")]
    [Tooltip("低体力時に各部位へ生成する継続VFX。ボススタン用の P_FX_Electrication を想定")]
    ...
    [SerializeField] private float m_healthRatioThreshold = 0.357f;
    ...
    [Label("スケール倍率")]
    [Tooltip("自動計算後に掛ける全体倍率。各部位の倍率と掛け合わせる")]
    [Min(0f)]
    [SerializeField] private float m_scaleMultiplier = 0.4f;

    [Label("死亡中も表示")]
    [Tooltip("OFFならHP0の死亡中はエフェクトを止める")]
    [SerializeField] private bool m_showWhileDead = true;
    ...
    private void EnsureEffectInstances()
    {
        ...
        for (int i = 0; i < m_bodyPartEffects.Length; i++)
        {
            ...
            PrepareEffectInstanceForBodyPart(effectInstance);
    ...
    private void PrepareEffectInstanceForBodyPart(BodyPartEffectInstance instance)
    {
        if (instance?.effectInstance == null) return;

        var binders = instance.effectInstance.GetComponentsInChildren<BindShapeToParentSkinnedMesh>(true);
        foreach (var binder in binders)
        {
            if (binder == null) continue;

            var ps = binder.GetComponent<ParticleSystem>();
            if (ps != null)
            {
                var shape = ps.shape;
    ...
    private void ApplyParticleTuning(BodyPartEffectInstance instance)
    {
        ...
        for (int i = 0; i < instance.particleDefaults.Length; i++)
        {
            ...
            var shape = ps.shape;
            shape.rotation = defaults.shapeRotation + instance.settings.LocalEulerOffset;
